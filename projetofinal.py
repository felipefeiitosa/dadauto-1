...
lucros = [0, 100, 250, 500, 1000, 1500, 2000, 2500, 5000, 10000, 15000, 25000, 50000, 100000, 150000, 250000, 500000,
         1000000]
acumulado = 0
acertos = 0
...
perguntas_dificeis = [
   ...
   },
   {
       ...
       "opções": ["a) o peregrino", "b) o pequeno príncipe", "c) dom quixote", "d) harry potter e a pedra filosofal"],
       "resposta": "b"
   },
   {
...
for x in range(0, 6):
    ...
    z = input('Qual a sua resposta? ').lower().strip()
    ...
    if z == easy['resposta']:
        ...
        escolhas = input(
            f'\nParabéns {nome}, você passou! Pode escolher entre parar e levar R$ {lucros[acertos]},00 ou continuar e tentar ganhar R$ {lucros[acertos + 1]},00. (p/parar | c/continuar): '
        ).lower().strip()
...
if z == final['resposta']:
    ...
    escolhas = input(
        f'\nParabéns {nome}, você venceu! Pode escolher entre parar e levar R$ {lucros[acertos]},00 ou tentar a sorte maior com R$ {lucros[acertos + 1]},00. (p/parar | c/continuar): '
    ).lower().strip()


    if escolhas in ('c', 'continuar'):
        print('Coragem, vamos nessa!')
    else:
        print(f'\nParabéns pelos seus R$ {lucros[acertos]},00, sr(a) {nome}')
