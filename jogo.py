...
# FELIPE GABRIEL: Tratamento de Erro
...
# FELIPE FELICIANO: RANKING

def salvar_ranking():
    with open("ranking_geral.txt", "w", encoding="utf-8") as arquivo:
        for i, jogador in enumerate(sorted(ranking, key=lambda x: x['pontuacao'], reverse=True), start=1):
...
# VINICIUS RAFAEL: BANCO DE DADOS DE PERGUNTAS
...
# FELIPE GABRIEL VAI EXPLICAR QUE SE REPETE:
...
# FELIPE GABRIEL: DIFERENÇA NA PERGUNTA FINAL
...
# FELIPE FELICIANO: RANKING TOP 10
...
def ver_ranking():
    ...
    if not ranking:
        ...
        for i, jogador in enumerate(rank_geral[:10], start=1):  # adauto pediu os 10 primeiros
